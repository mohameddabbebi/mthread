...
typedef struct mthread_sem_s {
  bool is_initialized =false;
  atomic_int value;                     // compteur du sémaphore
  atomic_flag thread_list_spinlock;     // protège la liste d'attente
  mthread_list_t thread_list;           // liste des threads bloqués
} mthread_sem_t;
...
typedef struct mthread_cond_s {
  bool is_initialized = false;
  atomic_flag thread_list_spinlock;
  mthread_list_t thread_list;
} mthread_cond_t;
